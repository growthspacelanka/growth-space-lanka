...
const AiPackages = () => {
    const packages = content.services.ai.packages.items;
                ...
                <div className="max-w-[1440px] mx-auto">
                        ...
                        {packages.map((pkg, index) => (
                            ...
                            >
                                ...
                                <div className="mb-6">
                                    ...
                                    {'monthlyPrice' in pkg && pkg.monthlyPrice && (
                                        <p className="text-lg md:text-xl font-normal text-accent tracking-tight">
                                            {pkg.monthlyPrice.includes(' / month') ? (
                                                <>
                                                    {pkg.monthlyPrice.replace(' / month', '')} /{' '}
                                ...
                                >
                                    {pkg.name === 'Custom E-commerce' ? 'Book a Call' : 'Proceed to Payment'}
                                </Link>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    >
                        <p className="text-xl md:text-2xl text-gray-600 font-light max-w-3xl mx-auto tracking-tight">
                            {content.services.ai.packages.pricingNote}
                        </p>
                    </motion.div>
                </div>
            </div>
